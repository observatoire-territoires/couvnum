...
- 2G : niveau de couverture TBC/BC/CL/NA.
- 4G : binaire oui-non
...
.pull-left[
- <img src="./img/evol_couv4G_BYT_072017_012019_v2.png" height="220px"  />
 <img src="./img/evol_couv4G_ORF_072017_012019_v2.png" height="220px"  />
- <img src="./img/evol_couv4G_SFR_072017_012019_v2.png" height="220px"  />
<img src="./img/evol_couv4G_FREE_072017_012019_v2.png" height="220px" />
]
.pull-right[
...
.pull-left[
```{r plot_nb_operateurs_detail_012019,echo=F, message=F, warning=F, fig.width=10,fig.height=9, fig.retina=2}
...
.pull-right[
<img src="./img/couv4G_nb_detail_operateurs_012019_v4.png" width=450 >
]
...
.pull-left[
```{r plot_nb_operateurs_detail_012019_vs,echo=F, message=F, warning=F, fig.width=10,fig.height=9, fig.retina=2}
...
ggplot(stats_croiz_superfice_nboperateurdetail %>%
  ...
  geom_treemap_text( layout = "fixed", grow = F, reflow = T, colour = "black") +
  geom_treemap_subgroup_border(layout = "fixed", colour = "black", size =1) +
  scale_fill_manual(labels = c('4 opérateurs : 78,3%',"3 opérateurs : 14,5%","2 opérateurs : 4,0%", "Orange uniquement : 1,8%","SFR uniquement : 0,1%", "ByTel uniquement : 0,1%","Free uniquement : 0,0%", "Zone blanche : 1,2%"), values = c('white',"white","white", "orangered","red","dodgerblue","green", "white"), name ="", guide=guide_legend(reverse=T)) +
  theme_ipsum() +
  theme(axis.text.y = element_blank(),
        legend.text=element_text(size=15),
        strip.text.x = element_text(face = "bold"),
...
```{r calcul_opemieuxdisant_comm, echo=F, message=F, warning=F, include = F, eval=TRUE}

################################
# indicateur part de l'opérateur 4G mieux-disant par commune

COMM_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 <-
  COMM_COUVMOB_4G_4OPE_012019.stat.FRMET.indics %>%
...
.pull-left[
```{r plot_opemieuxdisant_actuel_comm,eval=T, echo=F, message=F, warning=F, fig.width=8,fig.height=7.5, fig.retina=2}


# carte choroplèthe 
ggplot() +
  geom_sf(data = COMMG_COG2016_METDOM %>% 
            full_join(COMM_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 , 
...
```{r plot_opemieuxdisant_evol_comm,eval=T, echo=F, message=F, warning=F, fig.width=8,fig.height=7.5, fig.retina=2}
ggplot() +
...
EPCI_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 <-
...
ggi_choro <-
  ...
  geom_sf_interactive(data = geo_EPCI_poly_FRMETDOM %>% 
                        full_join(EPCI_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 %>%
                                    filter(an %in% '2019'), 
                                  by = c("EPCI" ="EPCI" ) ) %>%
...
ggplot() +
  ...
  labs(
    ...
    subtitle = "Entre juillet 2017 et janvier 2019, par EPCI",
    caption = "Source : Arcep, opérateurs mobiles, janvier 2019"
  ) +   
  theme_ipsum() + 
  theme(legend.position = "right",
...
# Evolution de la couverture par région


```{r calcul_reg, echo=F, message=F, warning=F, include = F}

REG_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 <-
  COMM_COUVMOB_4G_4OPE_072017.stat.FRMET.indics %>% mutate(an = 2017) %>%
  rbind.data.frame(COMM_COUVMOB_4G_4OPE_012019.stat.FRMET.indics %>% mutate(an = 2019) ) %>%
  rbind.data.frame(COMM_COUVMOB_4G_4OPE_012019.stat.FRDOM.indics %>% mutate(an = 2019)) %>%
  #rbind.data.frame(COMM_COUVMOB_4G_4OPE_012019.stat.FRDOM.indics) %>%
  ...
  top_n(2) %>%
  filter(pct.superficie == min(pct.superficie)) %>%
  distinct(REG, techno, pct.superficie, .keep_all = TRUE) %>%
  ungroup() %>%
...
```{r plot_reg,echo=F, message=F, warning=F, fig.height=4.5}


ggi_slope_reg <-
ggplot(data=REG_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 %>%
         left_join( REG_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 %>%
         ...
         mutate(tip = paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                            ...
                            "en juillet 2017 : ", "<b>",percent(pct.superficie.072017, accuracy = 0.1) ,"</b>" , "<br>",
                            "en janvier 2019 : ","<b>",percent(pct.superficie.012019, accuracy = 0.1) ,"</b>","</font>") ),
  ...
  #           #hjust = 2,
  #           size = 3) +
  geom_text_repel(data=REG_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 %>% filter(an ==2017),
                  aes(x=an, y = pct_superf_opemax,
                      #color = pct_superf_opemax,
                      label=LIB_REG),
    size = 2.1,
  ...
  geom_text(data=REG_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 %>% filter(an ==2017),
            aes(x=an, y = pct_superf_opemax,
                #color = pct_superf_opemax,
            ...
            color = "grey30",
            size = 2) +
  geom_text(data=REG_COUVMOB_4G_4OPE_012019.stat.FR.indics.max2 %>% filter(an ==2019),
            aes(x=an, y = pct_superf_opemax,
                #color = pct_superf_opemax,
                label=paste0( round(pct_superf_opemax * 100,0), "%")),
            #position = position_stack(vjust = 0.5),
 ...
 # scale_color_manual(values=c( "#84312D","#FD817F","#9DC86C", "#439B45"), name = 'Type de densité',guide=guide_legend(reverse=F)) +
  ...
  #coord_flip() +
  labs(
    title = "Part de la surface couverte en 4G par a minima deux opérateurs",
    #subtitle = "par région administrative",
    caption = "Source : Arcep, opérateurs mobiles, juillet 2017/janvier 2019"
  )
...
x <- girafe(ggobj = ggi_slope_reg, height_svg = 4.8)
x <- girafe_options(x, 
...
widgetframe::frameWidget(x, width = "70%", height = 5)
...
<iframe width='700' height='500' src='https://www.observatoire-des-territoires.gouv.fr/outils/cartographie-interactive/?view=map26&indics=couverture_tel.couv4g&serie=2019&lang=fr&iframe=1'></iframe>


---


# Couverture 4G des équipements publics

<img src="./img/ELIGFIXE_012019_nboperateurs_v1.png" width=500>


<img src="./img/ELIGFIXE_012019_tableauequipements_FRTOT_v2.png" width=700>

dans les outre-mer :

<img src="./img/ELIGFIXE_012019_tableauequipements_DOM_v2.png" width=700>
