...
.pull-left[
```{r plot_nb_operateurs_detail_072017,echo=F, message=F, warning=F, fig.width=10,fig.height=8, fig.retina=2}
...
```{r plot_opemieuxdisant_epci, echo=F, message=F, warning=F, fig.width=6}
...
ggi_choro <-
  ...
  geom_sf_interactive(data = geo_EPCI_poly_FRMETDOM %>% 
                        full_join(EPCI_COUVMOB_4G_4OPE_012019.stat.FR.indics.max %>%
                                    filter(an %in% '2019') %>%
                                    left_join(EPCI_COUVMOB_4G_4OPE_EVOL.stat.FRMET.indics.max %>%                                        select(EPCI,pct.superficie.012019, pct.superficie.072017 ),
                                              by = "EPCI"),  
                                  by = c("EPCI" ="EPCI" ) ) %>%
                      ...
                      # mutate(tip = paste0("<style> div.leaflet-popup-content {width:auto!important;}</style>",
                      #                       "<font size=2.5 color=white family=Roboto>","<b>",conv_accents(LIB_EPCI),"</b>","<br>","Part de la surface couverte par l'opérateur mieux-disant :","<br>",
...
.pull-right[
```{r plot_opemieuxdisant_epci_evol, echo=F, message=F, warning=F, fig.width=6}
...
---
# v2 avec graphiques ggiraph liés

```{r plot_opemieuxdisant_epci_evol_v2, echo=F, message=F, warning=F, fig.width=12}
...
<iframe src="http://outils.observatoire-des-territoires.gouv.fr/mob_resid/" width="650px" height="450px" frameBorder="0"></iframe>
